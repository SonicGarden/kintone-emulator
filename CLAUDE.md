...
pnpm run exec         # サーバー起動 + テスト実行 + サーバー停止を一括実行
```
...
- `fields` — フォームフィールド定義（`app_id`, `code`, `type`, `label`）
...
実装タスクを完了するには、以下のサイクルを繰り返す:

1. コードを実装する
2. `pnpm build && pnpm run exec` でテストをすべてパスさせる
3. `code-reviewer` エージェントにレビューさせる（`.claude/agents/code-reviewer.md` に定義。Task ツールの `subagent_type` では起動不可。「code-reviewer エージェントでレビューして」と会話で指示すると自動委譲される）
