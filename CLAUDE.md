...
pnpm build            # プロダクションビルド（core → server の順）
...
開発時は `packages/server/tsconfig.json` の paths で `@kintone-emulator/core/*` を `../core/src/*` に解決（ホットリロード対応）。本番ビルド時は `packages/core/dist/` を参照。
...
- **`comments.ts`** — コメントの CRUD 操作（findRecordExists, insertComment, deleteComment）
