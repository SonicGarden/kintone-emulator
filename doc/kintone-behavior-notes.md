...
- どちらも **UI のルックアップピッカー（参照先レコード選択ダイアログ）の初期絞り込み条件 / 初期並び順** にしか使われない。ユーザーは UI でこれを変更できる
- **REST API には影響しない**: `relatedKeyField` は `unique: true` 必須（非 unique は `GAIA_LO03` で deploy 拒否）のため、API 経由ではキー値ごとにマッチが最大 1 件しかなく、絞り込みも並べ替えも実質的に無意味
- 公式ヘルプも「マッチが 1 件のときは filterCond は適用されない」と明記
- 当エミュレーターは `filterCond` / `sort` のフィールド値を**ストレージするだけで参照しない**（実機 REST API と同じ挙動）。deploy 時のクエリ構文/型バリデーション（`CB_VA01` / `GAIA_IQ03` / `GAIA_IQ11`）も未実装
...
- `filterCond` / `sort` / `lookupPickerFields` — いずれも UI のルックアップピッカー専用で REST API 挙動には影響しない（上の §10 「`filterCond` / `sort` の役割」参照）
...
> 注: 以前は RECORD_NUMBER 参照 / SUBTABLE 内のルックアップは未対応だったが、いずれも実装済み（上記参照）。
