...
- **実 kintone 準拠のエラーレスポンス** — `CB_VA01` / `GAIA_RE01` / `GAIA_AP01` / `GAIA_BL01` / `GAIA_RE02` を `Accept-Language` で ja/en 切替
...
- **`LOOKUP` フィールドの自動コピー挙動** — `lookup.fieldMappings` 等の設定は保存されるだけで、レコード登録時にコピー元フィールドから値をコピーしない
...
- **CREATED_TIME / UPDATED_TIME / CREATOR / MODIFIER** — 自動補完される値は簡易的（`CREATOR` / `MODIFIER` はエミュレーター固定値）
