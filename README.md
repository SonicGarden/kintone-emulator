...
- **JS カスタマイズのテスト対応** — `@sonicgarden/kintone-emulator/hook-test` で `kintone.events.on(...)` を使ったカスタマイズコードをブラウザなしでテスト可能（`kintone.api()` 呼び出しはエミュレーターにプロキシ）
...
- **実 kintone 準拠のエラーレスポンス** — `CB_VA01` / `CB_NO02` / `GAIA_RE01` / `GAIA_AP01` / `GAIA_BL01` / `GAIA_RE02` / `GAIA_LO04` / `GAIA_IL01` / `GAIA_IL23` を `Accept-Language` で ja/en 切替
- **ゲストスペース** — `/k/guest/{spaceId}/v1/...` パス、ゲスト app への非ゲストパスアクセスで `GAIA_IL23`、ゲストパス × 通常 app で `CB_NO02` を返す（実機準拠）
- **プロセス管理（簡易版）** — `setup/app.json` の `status` で states/actions を定義すると、レコードに `ステータス` フィールドが付与され、`PUT /k/v1/record(s)/status.json` でアクション実行による状態遷移が可能。クエリでも `ステータス in (...)` / `ステータス = "..."` で検索できる（作業者・filterCond・STATUS_ASSIGNEE は未対応）
- **Webhook** — `setup/webhook.json` で送信先 URL とイベントを登録すると、レコード操作時に kintone 互換の通知 JSON を実際に POST する。`ADD_RECORD` / `UPDATE_RECORD` / `DELETE_RECORD` / `ADD_RECORD_COMMENT` / `UPDATE_STATUS` に対応し、一括 API はレコード毎に 1 通ずつ送信。Webhook 受信側（連携サービス）のローカルテストに使える
...
| エントリポイント | 用途 |
|---|---|
| `@sonicgarden/kintone-emulator` | インプロセスサーバー起動（`startServer`） |
| `@sonicgarden/kintone-emulator/test-support` | テストヘルパー（`createBaseUrl`, `createApp` など） |
| `@sonicgarden/kintone-emulator/hook-test` | JS カスタマイズテスト用スタブ（`createKintoneStub`） |
...
| PUT | `/k/v1/record/status.json` | レコードのステータス更新（アクション実行） |
| PUT | `/k/v1/records/status.json` | レコードのステータス一括更新（最大 100 件・全件ロールバック） |
...
上記すべて、`/k/guest/{spaceId}/v1/...` 形式の**ゲストスペース版パス**でも同等にアクセス可能です（パスの spaceId と app の所属スペースが一致しない場合は実機と同じく `CB_NO02` / 403 を返す）。
...
| POST | `/[session]/setup/app.json` | テスト用アプリの作成（`name`, `properties`, `layout`, `status`, `records`, `webhooks` を指定可能）。レスポンスに `app`, `revision`, `recordIds` を返す。`properties` のシステムフィールド（RECORD_NUMBER / CREATED_TIME / UPDATED_TIME）は明示していなければ自動補完される（既定コード: `レコード番号` / `作成日時` / `更新日時`）。`webhooks: [{url, events}]` を渡すとアプリ作成と同時に Webhook を登録できる（初期 `records` では発火しない） |
| POST | `/[session]/setup/auth.json` | 認証ユーザーの登録（`username`, `password`）。1人以上登録すると認証が有効になる |
| POST | `/[session]/setup/space.json` | テスト用スペースの登録（`{id?, isGuest?, name?}`）。`id` 省略で自動採番。`isGuest: true` でゲストスペース扱い。`setup/app.json` の `spaceId` / `threadId` と組み合わせてゲストスペース挙動を再現できる |
...
| POST / DELETE | `/[session]/setup/webhook.json` | Webhook 登録。`{app, webhooks: [{url, events}]}` を POST すると、以降そのアプリのレコード操作時に通知 JSON を送信先 URL へ POST する。同一 app への再登録は置き換え（冪等）。DELETE（`?app=...` または `{app}`）で解除 |
...
環境変数 `LOG_LEVEL` でリクエスト/レスポンスのログ出力を制御できます。

| 値 | 出力内容 |
|---|---|
| 未設定 / `off` | ログなし（デフォルト） |
| `info` | メソッド、URL、ステータスコード、処理時間 |
| `verbose` | 上記 + リクエストボディ、レスポンスボディ |

```sh
LOG_LEVEL=verbose pnpm dev    # リクエスト/レスポンスの詳細ログを表示
LOG_LEVEL=info pnpm dev       # リクエスト概要のみ表示
```
...
2. 以下の環境変数を設定（`VITE_` プレフィックス必須、vite のデフォルト挙動を利用しているため）:
   ...
   | `VITE_KINTONE_TEST_SPACE_APP_IDS` | `1:17` | 通常スペース所属アプリ。`spaceId:appId` 形式のカンマ区切り |
   | `VITE_KINTONE_TEST_GUEST_SPACE_APP_IDS` | `2:15` | ゲストスペース所属アプリ。同上 |
   ...
   `VITE_KINTONE_TEST_SPACE_APP_IDS` / `VITE_KINTONE_TEST_GUEST_SPACE_APP_IDS` はゲストスペース挙動の dualMode テスト用。`createTestSpaceApp({ kind: "space" \| "guestSpace", spaceIndex?, appIndex? })` から参照される。同一スペースに複数アプリを置く場合は `1:17,1:18`、別スペースを増やす場合は `1:17,3:20` のように記述。
...
## kintone JS カスタマイズのテスト

kintone の JS カスタマイズ（`kintone.events.on(...)` を使って画面の挙動をカスタマイズするコード）を Node.js / vitest で単体テストするためのスタブを `@sonicgarden/kintone-emulator/hook-test` で提供しています。ブラウザなしに、イベントの発火・フォームデータの操作・REST API 呼び出しまでテストできます。

### 基本的な使い方

カスタマイズコードを **setup 関数として書く** のが最もシンプルな形です。`kintone` を引数で受け取るようにすると、テスト時にスタブを渡せます。

```ts
// my-customization.ts（テスト対象）
import type { KintoneStub } from "@sonicgarden/kintone-emulator/hook-test";

export const setup = (kintone: KintoneStub) => {
...
```ts
// my-customization.test.ts
import { expect, test } from "vitest";
import { createKintoneStub } from "@sonicgarden/kintone-emulator/hook-test";
import { setup } from "./my-customization";

test("名前が空のとき保存できない", async () => {
...
### kintone.api() を使うカスタマイズのテスト

`createKintoneStub({ baseUrl })` に emulator の URL を渡すと、カスタマイズ内の `kintone.api()` 呼び出しがエミュレーターにプロキシされます。

```ts
import { afterEach, beforeAll, beforeEach, expect, test } from "vitest";
import { createKintoneStub } from "@sonicgarden/kintone-emulator/hook-test";
import { createApp, createBaseUrl, finalizeSession, initializeSession } from "@sonicgarden/kintone-emulator/test-support";
import { setup } from "./my-customization";

const SESSION = "my-customization-test";
let BASE_URL: string;
...
test("保存時に関連レコードが作成される", async () => {
  ...
  await stub.events.fire("app.record.create.submit", {
    record: { name: { type: "SINGLE_LINE_TEXT", value: "テスト" } },
  });
...
#### `createKintoneStub(options?)`

| オプション | 型 | 説明 |
|---|---|---|
| `baseUrl` | `string` | エミュレーターのセッション URL。`kintone.api()` のプロキシ先 |
| `appId` | `number` | `kintone.app.getId()` が返すアプリ ID の初期値 |

#### イベント操作

| メソッド | 説明 |
|---|---|
| `stub.events.on(event, handler)` | イベントハンドラーを登録（配列で複数イベントも可） |
| `stub.events.off(event, handler?)` | ハンドラーを解除（`handler` 省略で全解除） |
| `stub.events.fire(event, initial?)` | イベントを発火してハンドラーチェーンを実行。`false` または `KintoneEventObject` を返す |
| `stub.events.clear()` | 全ハンドラーをクリア |

#### API 呼び出し

| メソッド | 説明 |
|---|---|
| `stub.api(url, method, params?)` | REST API を呼び出す（`baseUrl` を渡した場合はエミュレーターにプロキシ） |
| `stub.api.url(path)` | `/k/v1/records` → `/k/v1/records.json` に変換（`kintone.api.url()` 互換） |

#### フォームデータ・画面情報

| メソッド | 説明 |
|---|---|
| `stub.app.record.get()` | 現在のフォームレコードを取得 |
| `stub.app.record.set({ record })` | フォームレコードを更新 |
| `stub.app.getId()` | アプリ ID を返す |
| `stub.app.getRecordId()` | レコード ID を返す |
| `stub.app.getQuery()` | 現在のクエリ文字列を返す（`_setQuery()` で設定） |
| `stub.app.getQueryCondition()` | `order by` / `limit` / `offset` を除いたクエリ条件を返す |
| `stub.app.getHeaderMenuSpaceElement()` | 一覧ヘッダーメニューの要素スタブを返す（`children` で追加済み要素を検証可能） |
| `stub.app.getHeaderSpaceElement()` | 一覧ヘッダーの要素スタブを返す |

#### テスト補助

| メソッド | 説明 |
|---|---|
| `stub._setAppId(id)` | アプリ ID を設定 |
| `stub._setRecordId(id)` | レコード ID を設定 |
| `stub._setRecord(record)` | フォームレコードを直接置き換え |
| `stub._setQuery(query)` | `getQuery()` / `getQueryCondition()` が返すクエリを設定 |
...
`setup/webhook.json` で送信先 URL とイベントを登録すると、対象アプリのレコード操作時に kintone 互換の通知 JSON を送信先へ実際に POST します。Webhook 受信側（連携サービス）をローカルでテストするためのフックです。

```ts
// 送信先 URL とイベントを登録（同一 app への再登録は置き換え）
await fetch(`${BASE_URL}/setup/webhook.json`, {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    app,
    webhooks: [
      { url: "http://localhost:9999/webhook", events: ["ADD_RECORD", "UPDATE_RECORD"] },
    ],
  }),
});

// レコード追加 → 登録 URL へ ADD_RECORD 通知が POST される
await client.record.addRecord({ app, record: { title: { value: "hello" } } });

// 解除
await fetch(`${BASE_URL}/setup/webhook.json?app=${app}`, { method: "DELETE" });
```

アプリ作成と同時に登録することもできます（`setup/app.json` の `webhooks`）。この場合、同時に作る初期 `records` では Webhook は発火しません（実機同様、設定後の操作のみが対象）。

```ts
await fetch(`${BASE_URL}/setup/app.json`, {
  method: "POST",
  headers: { "Content-Type": "application/json" },
  body: JSON.stringify({
    name: "テストアプリ",
    properties: { title: { type: "SINGLE_LINE_TEXT", code: "title", label: "タイトル" } },
    webhooks: [{ url: "http://localhost:9999/webhook", events: ["ADD_RECORD"] }],
  }),
});
```

対応イベントと通知 JSON（実機の `type` 別ペイロードに準拠）:

| `type` | 主なキー |
|---|---|
| `ADD_RECORD` / `UPDATE_RECORD` / `UPDATE_STATUS` | `id`, `type`, `app:{id,name}`, `record`（getRecord と同形式・`$id`/`$revision` 含む）, `recordTitle`, `url` |
| `DELETE_RECORD` | `id`, `type`, `app:{id,name}`, `recordId`, `deletedBy:{code,name}`, `deletedAt` |
| `ADD_RECORD_COMMENT` | `id`, `type`, `app:{id,name}`, `recordId`, `comment:{id,text,createdAt,creator,mentions}`, `url`（`#record={id}&comment={commentId}`） |

- HTTP POST / `Content-Type: application/json` で送信。`id` は通知ごとの UUID
- レコード一括 API（`records.json` の POST/PUT/DELETE、`records/status.json`）は**レコード毎に 1 通**ずつ送信
- 登録した `events` に含まれるイベントのみ送信
- **配信は同期**（レコード操作 API のレスポンスを返す前に POST 完了を待つ）。配信失敗（接続エラー・非 2xx）は握りつぶし、レコード操作 API 自体は成功扱い
- セッション終了（`finalize`）で登録は自動的にクリアされる

> エミュレーター固有の差分: `recordTitle` はアプリのタイトルフィールド設定を持たないためベストエフォート近似（RECORD_NUMBER → 先頭の SINGLE_LINE_TEXT → recordId）。`deletedBy` は固定ダミーユーザー `{code:"emulator", name:"Emulator User"}`、`comment.creator` は空ユーザー（API トークン認証ではユーザー不在のため）。`url` はエミュレーターの baseUrl ベースの擬似 URL。
...
- **プロセス管理の高度な機能**: `/k/v1/record/assignees.json`（作業者の更新）、`STATUS_ASSIGNEE` の値返却、アクションの `filterCond` 評価、state ごとの assignee 解決ロジックは未対応。基本的なステータス遷移（`PUT /k/v1/record(s)/status.json`）とクエリは対応済み
...
- **作成者 / 更新者 / カテゴリー / 作業者（CREATOR / MODIFIER / CATEGORY / STATUS_ASSIGNEE）** — 実機はアプリ作成時にこれらのシステムフィールドが自動的に存在し、レコード取得時もフィールドコードで値が返るが、本エミュレーターは未対応:
  - `setup/app.json` での自動補完は RECORD_NUMBER / CREATED_TIME / UPDATED_TIME のみ
  - レコード取得レスポンスに含まれない
  - `CREATOR` / `MODIFIER` をフィールドとして明示的に追加した場合も値は埋まらない
  - `STATUS_ASSIGNEE` の更新はできない
- **STATUS（ステータス）** — `setup/app.json` の `status.enable=true` のときのみ `ステータス` フィールドが付与される。プロセス管理が無効なアプリではレスポンスに `ステータス` キーは含まれない（実機と一致）
