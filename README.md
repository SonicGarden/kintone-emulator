...
- **ルックアップ（LOOKUP）対応** — POST/PUT 時に参照先アプリからキー検索して `fieldMappings` に従い自動コピー
- **実 kintone 準拠のエラーレスポンス** — `CB_VA01` / `GAIA_RE01` / `GAIA_AP01` / `GAIA_BL01` / `GAIA_RE02` / `GAIA_LO04` を `Accept-Language` で ja/en 切替
...
`relatedKeyField` に `レコード番号`（RECORD_NUMBER 型フィールド）を指定した参照もサポート。ただし `filterCond` / `sort` による参照先絞り込みや SUBTABLE 内ルックアップは未対応。
...
| POST | `/[session]/setup/app.json` | テスト用アプリの作成（`name`, `properties`, `layout`, `status`, `records` を指定可能）。レスポンスに `app`, `revision`, `recordIds` を返す。`properties` に RECORD_NUMBER 型を指定しなかった場合は `レコード番号` フィールドが自動補完される |
...
- **`CALC` フィールドの計算式評価** — `expression` は保存されるだけで式は評価されない
- **`LOOKUP` の `filterCond` / `sort`** — 参照先レコードの絞り込み・ソートは未対応（Phase 1 では基本コピー挙動のみ実装）
...
基本的な演算子（`=`, `!=`, `<`, `>`, `<=`, `>=`, `and`, `or`, `order by`, `limit`, `offset`）や `$id`、`NOW()` は動作しますが、以下は未検証または未対応:

- `like "%keyword%"`（部分一致）
- `in (v1, v2, ...)` / `not in (...)`
- kintone 独自の関数 `TODAY()` / `FROM_TODAY(1, DAYS)` / `THIS_MONTH()` / `LOGINUSER()` 等
- 構文エラー時のレスポンスは実装されているが、`id` が固定値で `errors` オブジェクトを含まない
...
- **作成日時 / 更新日時 / 作成者 / 更新者 / カテゴリー / ステータス / 作業者（CREATED_TIME / UPDATED_TIME / CREATOR / MODIFIER / CATEGORY / STATUS / STATUS_ASSIGNEE）** — 実機はアプリ作成時にこれらのシステムフィールドが自動的に存在し、レコード取得時もフィールドコード（既定は `作成日時` 等）で値が返るが、本エミュレーターは未対応:
  - フィールド定義に現れない（`setup/app.json` での自動補完は RECORD_NUMBER のみ）
  - レコード取得レスポンスに含まれない
